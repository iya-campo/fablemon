...
import { Album, CatchingPokemon, Dashboard, Diamond, Explore, Forest, Medication, MedicationLiquid, Park, Tsunami, Volcano } from '@mui/icons-material'
import { Box, Button, Divider, FormControl, Grid, List, ListItemButton, ListItemIcon, ListItemText, MenuItem, Paper, Select, SelectChangeEvent, Stack, SvgIcon, Typography } from '@mui/material'
import Image from 'next/image'
import EncounterActions from '@/components/EncounterActions';
import WildBattle from '@/app/wilderness/WildBattle';
...
const Wilderness = () => {
  ...
  */}
  ...
  const handleChangeHabitat = (event: SelectChangeEvent) => {
    setHabitat(event.target.value as string);
  }
  
  const handleOpenEncounter = () => setEncounter('Bulbasaur');
  const handleCloseEncounter = () => setEncounter('');
  ...
  const renderResults = () => {
    ...
    if (status === 'explored') return(
      <Typography variant='caption' textAlign='center' fontWeight='bold'>{`You wandered through the ${habitat}.`}</Typography>
    )
  }
  ...
  return (
    <>
      <WildEncounter
        name='Pikachu'
        img='https://raw.githubusercontent.com/PokeAPI/sprites/refs/heads/master/sprites/pokemon/other/showdown/25.gif'
        handleStartBattle={handleStartBattle}
        open={!!encounter}
        handleClose={handleCloseEncounter}
      />
      <WildBattle battleStart={battleStart} handleEndBattle={handleEndBattle} />
      <Stack display='flex' gap={2} mb={6}>
        ...
        <Grid container size={12} direction={{ xs: 'column-reverse', sm: 'row' }} rowGap={4} bgcolor='#ddd' borderRadius={2} sx={{ backgroundImage: `url(images/forest.jpg)`, backgroundPositionY: '-150px' }} p={2}>
          <Grid size={{ xs: 12, sm: 6, md: 4 }}>
            <Paper sx={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', height: '100%', py: 4 }}>
          ...
          <Grid container size={{ xs: 12, sm: 6, md: 8 }} justifyContent='flex-end'>
            <FormControl sx={{ width: { xs: '100%', md: 200 }, ml: { xs: 0, sm: 2 } }}>
              <Select
        ...
        <Grid container size={12} spacing={{ xs: 2, lg: 4 }} rowGap={4} mt={2}>
          ...
          <Grid container size={{ xs: 12, md: 7 }} spacing={0}>
            ...
            <Stack direction='row' justifyContent='flex-end' spacing={0.5} mb={1} width='100%'>
              <Button variant='contained' size='small' disableElevation sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
